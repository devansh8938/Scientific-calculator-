...
def SearchData():
    ...
    Database(search_keyword)

def Database(search_keyword=None):
    conn = sqlite3.connect("pythontut.db")
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, age TEXT, address TEXT, contact TEXT)")
    
    if search_keyword:
        cursor.execute("SELECT * FROM `member` WHERE `firstname` LIKE ? OR `lastname` LIKE ? OR `age` LIKE ? OR `gender` LIKE ? OR `contact` LIKE ? OR `address` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%'))
    else:
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
...
# ...
def Database(search_keyword=None, filter_criteria=None):
    conn = sqlite3.connect("pythontut.db")
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, age TEXT, address TEXT, contact TEXT)")
    
    if search_keyword and filter_criteria:
        if filter_criteria == "Contact":
            cursor.execute("SELECT * FROM `member` WHERE `contact` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        elif filter_criteria == "Last Name":
            cursor.execute("SELECT * FROM `member` WHERE `lastname` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        elif filter_criteria == "First Name":
            cursor.execute("SELECT * FROM `member` WHERE `firstname` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        elif filter_criteria == "Address":
            cursor.execute("SELECT * FROM `member` WHERE `address` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        elif filter_criteria == "Age":
            cursor.execute("SELECT * FROM `member` WHERE `age` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        elif filter_criteria == "Gender":
            cursor.execute("SELECT * FROM `member` WHERE `gender` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%',))
        else:
            cursor.execute("SELECT * FROM `member` WHERE `firstname` LIKE ? OR `lastname` LIKE ? OR `age` LIKE ? OR `gender` LIKE ? OR `address` LIKE ? OR `contact` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%'))
    elif search_keyword:
        cursor.execute("SELECT * FROM `member` WHERE `firstname` LIKE ? OR `lastname` LIKE ? OR `age` LIKE ? OR `gender` LIKE ? OR `address` LIKE ? OR `contact` LIKE ? ORDER BY `lastname` ASC", ('%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%', '%'+search_keyword+'%'))
    elif filter_criteria:
        if filter_criteria == "A to Z":
            cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        elif filter_criteria == "Z to A":
            cursor.execute("SELECT * FROM `member` ORDER BY `lastname` DESC")
        elif filter_criteria == "Contact":
            cursor.execute("SELECT * FROM `member` ORDER BY `contact` ASC")
        elif filter_criteria == "Last Name":
            cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        elif filter_criteria == "First Name":
            cursor.execute("SELECT * FROM `member` ORDER BY `firstname` ASC")
        elif filter_criteria == "Address":
            cursor.execute("SELECT * FROM `member` ORDER BY `address` ASC")
        elif filter_criteria == "Age":
            cursor.execute("SELECT * FROM `member` ORDER BY `age` ASC")
        elif filter_criteria == "Gender":
            cursor.execute("SELECT * FROM `member` ORDER BY `gender` ASC")
        else:
            cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
    else:
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
...
#============================SEARCH FRAME===============================
SearchFrame = Frame(Top, width=500, bd=1, relief=SOLID)
SearchFrame.pack(side=TOP, pady=10)

lbl_text = Label(SearchFrame, text="Search:", font=('arial', 12), width=8, bg="#6666ff")
lbl_text.pack(side=LEFT, padx=10)

search_var = StringVar()
entry_search = Entry(SearchFrame, textvariable=search_var, font=('arial', 12), width=15)
entry_search.pack(side=LEFT)

btn_search = Button(SearchFrame, text="Search", command=SearchData)
